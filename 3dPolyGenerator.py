#Randome 3D polygenerator final product here
###ONLY NECESSARY TESTED FUNCTIONS AND CODE HERE
...
#Generates a random polygon in x, y and then adds a zero for z val and returns the list
#Currently running as intended, could be made more efficient
def randPolyGenerator(iters):
    listOfLayers = []
    for i in range(iters):
        vertices = random_polygon(num_points=random.randint(3, 7))
...
#Take in 2d coordinates and transform them across either the x or y axis and return the list
...
def planeMaker(coords, polyConnectCoords=None):
    ...
    orderedListFromConnectingPointToConnectedPoints = distanceFromPoint(pointToBeConnected, newNewCoords, 1)
    #List of points that will actually be getting connected to each other
    polyConnectCoords.append(connectedCoords(random.randint(2,3), pointToBeConnected, orderedListFromConnectingPointToConnectedPoints))
...
#Function that does all of the work of the functions above, just in one line instead
...
coordSet = rand3dPolyGen(random.randint(3,4))
centCoordSet = center(coordSet)
#print(f"Coordset: {coordSet}")

with open('listOfListsOfTuples.pkl', 'wb') as f:
    pickle.dump(centCoordSet, f)
